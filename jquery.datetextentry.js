...
(function ($) {
     ...
     * ============================== */
    var isTouch = navigator.userAgent.toString().search(/iPad/i) > -1;
        ...
        , build_ui: function () {
            ...
            if (isTouch === true) {
                ...
                var fixBlur = function () {
                    ...
                    //console.log('FOCUS active element is now: ' + document.activeElement.tagName + '_' + document.activeElement.className);
                    if (self.on_blur && self.hasFocus === true && document.activeElement.className.indexOf('jq-dte-') === -1) {
                        self.on_blur();
                        self.hasFocus = false;
                    }
                }

                window.addEventListener ? window.addEventListener('focus', fixBlur, true) : window.attachEvent('onfocusout', fixBlur);
                window.addEventListener ? window.addEventListener('click', fixBlur, true) : window.attachEvent('onclick', fixBlur);
            }
        }
